...
const ProductOptionsDropdown: React.FC<Props> = ({ productId, onOptionsChange, onPriceUpdate }) => {
  ...
  // Get the mutation and loading states from useUpdatePrice hook
  const { mutation } = useUpdatePrice(onPriceUpdate); // Access the mutation directly
  ...
  const handleChange = async (group: string, optionId: number) => {
    ...
    if (onPriceUpdate) {
      ...
      mutation.mutate({ productId, optionIds: selectedOptions }); 
    }
  };
