...
const ShippingCalculator: React.FC<ShippingCalculatorProps> = ({ productId, selectedOptions, sinaliteId }) => {
  ...
  const fetchShippingEstimate = async () => {
    ...
    try {
      ...
      const res = await fetch(`http://192.168.0.100:8000/api/v1/shipping-details/${productId}/${optionChain}?sinaliteId=${sinaliteId}`);
  ...
  return (
    <div className="p-4 border rounded-lg shadow-md">
      ...
      {loading ? (
        ...
        <div>
          <p><strong>Weight per Box:</strong> {shippingData.weight_per_box} lbs</p>
          <p><strong>Box Size:</strong> {shippingData.box_size}</p>
          <p><strong>Package Size:</strong> {shippingData.package_size}</p>
          <p className="font-bold"><strong>Estimated Shipping Cost:</strong> ${shippingData.shipping_cost}</p>
        </div>
      ) : (
        <div className="space-y-2">
          <input type="text" name="city" value={form.city} onChange={handleChange} placeholder="City" className="border p-2 w-full" />
          <input type="text" name="state" value={form.state} onChange={handleChange} placeholder="State" className="border p-2 w-full" />
          <input type="text" name="postalCode" value={form.postalCode} onChange={handleChange} placeholder="Postal Code" className="border p-2 w-full" />
          <button onClick={fetchShippingEstimate} className="mt-2 bg-blue-500 text-white py-2 px-4 rounded">
            Get Estimate
          </button>
        </div>
      )}
