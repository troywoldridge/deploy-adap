...
const Notepad25Page: React.FC = () => {
  // Set your actual product ID here
  const productId = 82; // <--- Change to the correct ID for 25-page notepads
    ...
    const getProductData = async () => {
      try {
        const res = await fetch(
          `http://192.168.0.100:8000/api/v1/products/${productId}`,
          { cache: "no-store" }
        );
        if (!res.ok) throw new Error(`Fetch failed with status ${res.status}`);
        const data: Product = await res.json();
        console.log("Raw product data (25-page):", data);
        setProductData(data);
        setUpdatedPrice(data.price);
      } catch (err) {
        if (err instanceof Error) {
          console.error("Error fetching product data (25-page):", err);
        } else {
  ...
  if (!productData) {
    return (
      <div className="container mx-auto px-4 py-24">
        <p className="text-center">Loading 25-page notepads...</p>
      </div>
    );
  }
  ...
  // Include the primary image as the first item in the gallery
