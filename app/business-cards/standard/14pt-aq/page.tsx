...
const Aq14Page: React.FC = () => {
  ...
  const handleShippingSubmit = async (formData: { country: string; city: string; postalCode: string }) => {
    try {
  ...
  const handlePriceUpdate = async () => {
    ...
    try {
      const response = await fetch(`http://192.168.0.100:8000/api/v1/products/${productId}/price?${query}`);
      const data = await response.json();
  ...
  return (
    <div className="container mx-auto px-4 py-24">
      <h1 className="text-3xl font-bold text-center mb-8">{productData.name} - 14pt AQ Business Cards</h1>
      ...
      <div className="mt-12">
        ...
        {shippingEstimate && <p className="text-green-600 font-semibold mt-4">{shippingEstimate}</p>}
      </div>
