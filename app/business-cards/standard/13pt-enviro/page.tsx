...
import { PricingOption } from "@/types"; // or wherever it lives
...
const EnviroPage: React.FC = () => {
  ...
  const fetchFinalPrice = async (updatedOptions: { [key: string]: number }) => {
    ...
    try {
      const priceRes = await fetch(
        `http://192.168.0.100:8000/api/v1/calculateFinalPrice`,
        {
          method: "POST",
          headers: {
  ...
  const weight = parseFloat(initialPricingData?.weight_per_box || "0") * parseInt(initialPricingData?.number_of_boxes || "1");
  const [length, width, height] = initialPricingData?.box_size?.split(" x ").map(Number) || [0, 0, 0];

  return (
    <div className="container mx-auto px-4 py-24">
      <h1 className="text-3xl font-bold text-center mb-8">{productData.name} - 13pt Enviro Business Cards</h1>
