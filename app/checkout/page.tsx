...
const CheckoutPage: React.FC = () => {
    ...
    return (
        <div className="checkout-page">
            ...
            <input type="text" value={shippingAddress.address} onChange={(e) => setShippingAddress({ ...shippingAddress, address: e.target.value })} placeholder="Address" />
            ...
            <input type="text" value={shippingAddress.zip} onChange={(e) => setShippingAddress({ ...shippingAddress, zip: e.target.value })} placeholder="ZIP Code" />
            ...
            {shippingRates && (
                <div className="shipping-options">
                    ...
                    {shippingRates.map((rate, index) => (
                        <div key={index}>
                            <input type="radio" name="shippingMethod" value={rate.method} checked={shippingMethod === rate.method} onChange={() => setShippingMethod(rate.method)} />
                            <label>{rate.method} - ${rate.price.toFixed(2)} (Est. {rate.estimated_days} days)</label>
                        </div>
                    ))}
                </div>
            )}
            ...
            <h2>Payment</h2>
            <input type="text" value={paymentToken} onChange={(e) => setPaymentToken(e.target.value)} placeholder="Payment Token" />
